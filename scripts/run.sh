...
podman build -t playbook_playground --build-arg UID=$UID --build-arg USER=$USER_CNT_NAME -f docker/Dockerfile.dest .
podman build -t playbook_ansible --build-arg UID=$UID --build-arg USER=$USER_CNT_NAME -f docker/Dockerfile.ansible .
